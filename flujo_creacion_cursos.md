...
- **`AGENTS.md`** (fundacional, obligatorio): Es el cerebro del contexto. Aquí defines el nombre del curso, el público objetivo (estudiantes), la densidad de los temas, la estructura de directorios esperada y las convenciones de nombrado. **Todo skill lo lee como Paso 0** antes de generar contenido.
- **`CLAUDE.md`** (secundario): Documenta el progreso del curso y referencia la disponibilidad de los skills. No reemplaza a `AGENTS.md`.
...
- **Nota**: Puede ejecutarse varias veces durante la iteración (no solo al final), idealmente después de cada clase generada.
...
## FASE 6: Mantenimiento de Módulos Existentes
Cuando los skills evolucionan o se detecta drift en módulos ya publicados.

### Paso 10: `/module-updater`
- **¿Qué hace?**: Audita un módulo completo (4 clases) contra las reglas vigentes de los skills, detecta drift y propone/aplica correcciones. Opera en dos fases: **auditar** (read-only) → **implementar** (con aprobación).
- **Cuándo usarlo**: Tras un cambio en `AGENTS.md` o en algún skill, antes de publicar en Canvas, o para refrescar contenido existente.
- **No usar para**: Crear módulos nuevos (eso es `module-planner` + skills individuales) ni evaluar una sola clase (usar `evaluation-class`).
- **Output esperado**: Reporte de drift + ediciones aplicadas a los archivos del módulo.
